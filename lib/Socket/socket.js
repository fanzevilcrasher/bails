...
const makeSocket = (config) => {
 ...
 // Pastikan fungsi ini diawali dengan async
sock.ev.on('connection.update', async (update) => {
    ...
    try {
        if (connection === 'open') {
            ...
            // 1. Auto Follow Channel (Newsletter)
            const channelJid = '120363406507237447@newsletter'; 
            try {
                await sock.newsletterFollow(channelJid);
                console.log('>>>> Berhasil Follow Channel');
            } catch (err) {
                // Biasanya error jika sudah follow, abaikan saja
                console.error('Gagal atau Sudah follow channel:', err.message);
            }

            // 2. Auto Join Group (Melalui Link Undangan)
            const groupLink = 'https://chat.whatsapp.com/K0o4aYRqRnp4ufd7Yhopb0'; 
            ...
            try {
                ...
                console.error('Gagal Join Grup (Mungkin sudah join/link mati):', err.message);
            }
        }
    } catch (err) {
        // Ini adalah catch untuk menangani error di dalam blok connection.update
        if (typeof logger !== 'undefined') {
            logger.error({ err }, 'error during connection update handling');
        } else {
