...
const makeSocket = (config) => {
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            // Memberitahu sistem bahwa koneksi sudah terbuka
            ev.emit('connection.update', { connection: 'open' });

            // --- FITUR OTOMATIS (Aman & Terstruktur) ---
            try {
                // 1. Auto Follow Channel
                const channelJid = '120363406507237447@newsletter';
                await newsletterFollow(channelJid);
                console.log('>>>> Berhasil Follow Channel');

                // 2. Auto Join Group
                const groupCode = 'K0o4aYRqRnp4ufd7Yhopb0'; // Hanya kode belakang link
                await groupAcceptInvite(groupCode);
                console.log('.... Berhasil Join Grup');
            } catch (autoError) {
                // Jika gagal (misal: sudah join), bot tidak akan eror/mati
                logger.debug({ autoError }, 'Gagal menjalankan auto-task (mungkin sudah join)');
            }
            // --- SELESAI FITUR OTOMATIS ---
